...
class PesananWithUserPaketAndPembayaran extends Model
{
    ...
    protected $allowedFields    = ['id_pesanan, id_user, id_paket, id_pembayaran, nama_lengkap, tanggal_mulai, tanggal_selesai, telp, alamat, catatan, extra_waktu_kerja, extra_premium_magazine, extra_background, extra_tempat, extra_orang, extra_wisudawan, email, nama_lengkap, no_telp, nama_paket, harga_paket, fotografer, videografer, asisten, waktu_kerja, jumlah_foto, jumlah_foto_edit, cetak_foto, videografi, penyimpanan, note, deskripsi_paket, order_id, jumlah_bayar, status, created_at, updated_at'];

    public function getPesananWithUserPaketAndPembayaran()
    {
        $db = db_connect();
        $builder = $db->table('pesanan');
        $builder->select('pesanan.id_pesanan, user.id_user, paket.id_paket, pembayaran.id_pembayaran, pesanan.nama_lengkap,  pesanan.tanggal_mulai, pesanan.tanggal_selesai, pesanan.telp, pesanan.alamat, pesanan.catatan, pesanan.extra_waktu_kerja, pesanan.extra_premium_magazine, pesanan.extra_background, pesanan.extra_tempat, pesanan.extra_orang, pesanan.extra_wisudawan, user.email, user.nama_lengkap, user.no_telp, paket.nama_paket, paket.harga_paket, paket.fotografer, paket.videografer, paket.asisten, paket.waktu_kerja, paket.jumlah_foto, paket.jumlah_foto_edit, paket.cetak_foto, paket.videografi, paket.penyimpanan, paket.note, paket.deskripsi_paket, pembayaran.order_id, pembayaran.jumlah_bayar, pembayaran.status, pesanan.created_at, pesanan.updated_at');
    ...
    public function getPesananWithUserPaketAndPembayaranById(int $id_pesanan)
    {
        $db = db_connect();
        $builder = $db->table('pesanan');
        $builder->select('pesanan.id_pesanan, user.id_user, paket.id_paket, pembayaran.id_pembayaran, pesanan.nama_lengkap,  pesanan.tanggal_mulai, pesanan.tanggal_selesai, pesanan.telp, pesanan.alamat, pesanan.catatan, pesanan.extra_waktu_kerja, pesanan.extra_premium_magazine, pesanan.extra_background, pesanan.extra_orang, user.email, user.nama_lengkap, user.no_telp, paket.nama_paket, paket.harga_paket, paket.fotografer, paket.videografer, paket.asisten, paket.waktu_kerja, paket.jumlah_foto, paket.jumlah_foto_edit, paket.cetak_foto, paket.videografi, paket.penyimpanan, paket.note, paket.deskripsi_paket, pembayaran.order_id, pembayaran.jumlah_bayar, pembayaran.status, pesanan.created_at, pesanan.updated_at');
        $builder->join('user', 'user.id_user = pesanan.id_user');
        $builder->join('paket', 'paket.id_paket = pesanan.id_paket');
        $builder->join('pembayaran', 'pembayaran.id_pesanan = pesanan.id_pesanan');
